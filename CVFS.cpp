...
void man(char *name)
{
  ...
  {
    printf("Description : Used to list all nfromation of files\n");
    printf("Usage : ls\n");
  }
...
void DisplayHelp()
{
  ...
  printf("exit : To Terminate file system file\n");
  printf("stat : To Display information of file using name\n");
  printf("fstat : To Diaplay information of file using file descriptor\n");
...
int CreateFile(char *name, int Permission)
{
   ...
   if((name == NULL) || (Permission == 0) || (Permission > 3))
   {
    ...
    if(Get_Inode(name) != NULL) //new file is exits or not this func will check it
      return -3;
    ...
    strcpy(UFDTArr[i].ptrfiletable->ptrinode->FileName,name);  //rasta
...
int ReadFile(int fd, char *arr, int isize)
{
  ...
  if(UFDTArr[fd].ptrfiletable->ptrinode->Permission != READ && UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)
  return -3;
...
int fstat_file(int fd)
{
  ...
  if(temp->Permission == 1)
     printf("File Perission : Read only\n");
...
int main()
{
    ...
    while(1)
    {
        ...
        if(count == 1)
        {
            ...
            {
                printf("Terminating the Marvellos Virtual File System\n");
                break;
            }
            else
            {
        ...
        else if(count == 2)
        {
            ...
            else if(strcmp(command[0],"write") == 0)
            {
                ...
                if(ret == -2)
                  printf("ERROR : There is no sufficent memoery to write\n");
                if(ret == -3)
        ...
        else if(count == 3)
        {
            if(strcmp(command[0],"create") == 0)
            {
              ...
              // ret = CreateFile(Demo.txt,3);
                ...
                if(ret == -3)
                   printf("ERROR : File alreday exixts\n");
                if(ret == -4)
